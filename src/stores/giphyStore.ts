...
export const useGiphyStore = defineStore('giphy', {
  ...
  actions: {
    async fetchTrending() {
      if (!this.apiKey) {
        Notify.create({ type: 'negative', message: 'Chave da API GIPHY não configurada.' });
        return;
      }
      ...
      try {
        ...
        Notify.create({ type: 'negative', message: 'Falha ao buscar trending GIFs.' });
      } finally {
    ...
    async fetchGifsByCategory(categoryName: string) {
        ...
        try {
            ...
            console.error(`Erro ao buscar GIFs para a categoria ${categoryName}:`, error);
            Notify.create({ type: 'negative', message: `Falha ao buscar GIFs para ${categoryName}.` });
        } finally {
    ...
    addFavorite(gif: Gif) {
      if (!this.isFavorite(gif.id)) {
        ...
        Notify.create({ type: 'positive', message: 'GIF adicionado aos favoritos!' });
      }
    },
